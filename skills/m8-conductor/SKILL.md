...
- Wrap advances `plan/state.json` to reflect the completed milestone (the documented cold-resume anchor must track status, not just `log.md` — M1 exit test caught it stale). Re-runs any gate whose evidence predates a later repair (stale-evidence check: engine fixed during playtest ⇒ refresh gate 2/3 evidence before closing; retro: emberwake/M0 gap 3).
